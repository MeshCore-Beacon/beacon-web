  ...
  useEffect(() => {
    if (!hadDataRef.current) {
      ...
      for (const p of packets) {
        seenHashesRef.current.add(p.packetHash);
      }
      return;
    }
    ...
    for (const p of packets) {
      if (seenHashesRef.current.has(p.packetHash)) break;
    ...
    if (seenHashesRef.current.size > LIVE_BUFFER_CAP * 2) {
      const keep = new Set(packets.map((p) => p.packetHash));
      seenHashesRef.current = keep;
    }
