...
export function CloseButton({ onClose, label = "Close", className }: {
  ...
  return (
    <button
      ...
      onClick={onClose}
      aria-label={label}
      className={`flex items-center justify-center w-9 h-9 md:w-7 md:h-7 rounded text-text-muted hover:text-text-bright hover:bg-white/5 cursor-pointer transition-colors ${className ?? ""}`}
    >
