    ...
    <div className="flex items-center flex-1 min-w-0">
      <Dropdown
        ...
        renderTrigger={({ toggle }) => (
          <button
            type="button"
            className="flex items-center gap-1 text-[11px] font-mono px-2 py-[3px] rounded-l-sm border border-r-0 border-border bg-bg-surface text-text-muted hover:text-text-normal transition-colors cursor-pointer"
            onClick={toggle}
          >
            {currentField.label}
      ...
      >
        {(close) => FIELDS.map((f) => (
          <button
            ...
            type="button"
            disabled={f.disabled}
            className={`w-full text-left px-2.5 py-1 text-[11px] font-mono transition-colors ${
              f.disabled
          ...
          >
            {f.label}
            {f.disabled && <span className="text-[9px] ml-1 text-text-dim/40">soon</span>}
          </button>
        ))}
      </Dropdown>
      ...
      <div className="relative flex-1 min-w-0">
        ...
        <input
          ...
          className="w-full text-[11px] font-mono bg-bg-surface border border-border rounded-r-sm pl-7 pr-7 py-[3px] text-text-bright placeholder:text-text-dim outline-none focus:border-primary-dim transition-colors"
        />
        {localValue && (
          <button
            type="button"
