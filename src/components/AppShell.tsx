...
function LiveBadge({ wsManager }: { wsManager: WsManager }) {
  ...
  if (status === "connected") {
    return (
      <div className="flex items-center gap-[5px] font-mono text-[11px] text-green bg-green/8 border border-green/15 px-2 py-[2px] rounded-sm">
  ...
  if (status === "connecting") {
    return (
      <div className="flex items-center gap-[5px] font-mono text-[11px] text-warn bg-warn/7 border border-warn/15 px-2 py-[2px] rounded-sm">
        STALE {staleStr}
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-[5px] font-mono text-[11px] text-danger bg-danger/8 border border-danger/15 px-2 py-[2px] rounded-sm">
      OFFLINE
    </div>
  );
}
    ...
    >
      {(close) => iatas ? (
        <>
          <button
            type="button"
            className={`w-full flex items-center gap-2.5 px-3 py-1.5 text-left text-[11px] font-mono transition-colors ${
              region === "*"
        ...
        <button
          type="button"
          className="w-full flex items-center gap-2.5 px-3 py-1.5 text-left text-[11px] font-mono text-text-bright bg-primary/10"
          onClick={() => {
            setRegion("*");
            localStorage.setItem("tower-region", "*");
            close();
...
export function AppShell({ activeTab, onTabChange, wsManager, children }: AppShellProps) {
  return (
    <div className="flex flex-col h-dvh">
      ...
      <nav className="flex bg-bg-surface border-b border-border px-4 shrink-0" role="tablist">
        {TABS.map((tab) => (
          <button
            ...
            className={`px-[18px] py-[9px] text-xs font-medium tracking-wider border-b-2 cursor-pointer transition-colors ${
              activeTab === tab
      ...
      <footer className="flex items-center px-4 py-[5px] bg-bg-surface border-t border-border font-mono text-[11px] text-text-dim shrink-0">
