...
export function HopBadge({ hop }: { hop: Observation["resolvedPath"][number] }) {
  if (hop.confidence === "high" && hop.node) {
    return (
      <span className={`px-1.5 py-[2px] rounded-sm border font-medium ${VARIANT_CLASSES.advert}`}>
        {hop.node.name}
      </span>
    );
  }
  if (hop.confidence === "ambiguous" && hop.candidates) {
    return (
      <span className={`px-1.5 py-[2px] rounded-sm border font-medium ${VARIANT_CLASSES.trace}`}>
        {hop.idBytes?.toUpperCase()} ({hop.candidates.length} candidates)
      </span>
    );
  }
  return (
    <span className={`px-1.5 py-[2px] rounded-sm border font-medium ${VARIANT_CLASSES.default}`}>
